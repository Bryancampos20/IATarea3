...
embeddings_respuestas = np.array([nlp(respuesta).vector for respuesta in respuestas])
...
# Definir las preguntas del usuario (10 escenarios)
preguntas_usuario = [
...
# Umbral de similitud mínimo aceptable
umbral_similitud = 0.5

# Procesar cada pregunta y encontrar la respuesta más similar
for pregunta in preguntas_usuario:
    embedding_pregunta = nlp(pregunta).vector
    ...
    # Penalización adicional para respuestas que se repiten con mucha frecuencia
    penalizacion = 1.0 - (similitudes.mean() * 0.1)  # Penaliza respuestas demasiado generales
    
    similitudes = similitudes * penalizacion
    ...
    if mejor_similitud >= umbral_similitud:
        respuesta_seleccionada = respuestas[indice_mejor_respuesta]
    else:
